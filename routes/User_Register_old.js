...
async function fetchMobileNumbers() {
  try {
    ...
    const reverseMobileNumbers = [...mobileNumbers].reverse(); // Create a copy and reverse it

    for (const mobileNumber of reverseMobileNumbers) {
      await pairMoneyDevide(mobileNumber); // a
    }
  } catch (error) {
    console.error("Error fetching or processing mobile numbers:", error);
  }
}
...
const pairMoneyDevide = async (mobileNumber) => {
  try {
    ...
    const leftCount = await countUsers(userData.leftUser); // 1
    const rightCount = await countUsers(userData.rightUser); // 1
    const totalTeam = leftCount + rightCount;
    const ratioString = `${rightCount} : ${leftCount}`; // 1:1
    const ratioArray = ratioString.split(" : ");
    const minSide = Math.min(parseInt(ratioArray[0]), parseInt(ratioArray[1])); // 1:1
    const maxSide = Math.max(parseInt(ratioArray[0]), parseInt(ratioArray[1])); // 1:1
    ...
    const leftCountU = await countTodayUsers(userData.leftUser, todayDate); // 1
    const rightCountU = await countTodayUsers(userData.rightUser, todayDate); // 1

    const minSideToday = Math.min(leftCountU, rightCountU);

    const userRole = userData.role;
    const roleAchive = userData.roleAchive;
    ...
    setTimeout(async () => {
      ...
      if (
        ...
        // const updateGreenWallet = userData.greenWallet + 11000;
        // const updatedTotalRewards = userData.totalRewards + 11000;
        // console.log(userData.fullName, userData.greenWallet, updateGreenWallet,  "Vip 1 Start")
      ...
      // }
      // // vip6
      // if (minSide >= 30 && userRole != "vip6" && userRole != "vip7") {
      //   const updateGreenWallet = userData.greenWallet + 110000;
      //   const updatedTotalRewards = userData.totalRewards + 110000;
      //   console.log(userData.fullName, userData.greenWallet, updateGreenWallet,  "Vip 1 Start")
      ...
      // }
      // // vip7
      // if (minSide >= 35 && (userData.role != "vip7" || userRole != "vip7")) {
      //   const updateGreenWallet = userData.greenWallet + 220000;
      //   const updatedTotalRewards = userData.totalRewards + 220000;
      //   console.log(userData.fullName, userData.greenWallet, updateGreenWallet,  "Vip 1 Start")
      ...
      //     userData.__v += currentGetRoyaltyCount * 100;
...
const addFakeUsersToDB = async (
  ...
  try {
    ...
    const newUser = new UserRegister({
      ...
      treeId: newTreeId, // b
      level: parentUser.level + 1,
      referredBy: referredByUser._id, // a
      adhaar,
      mlmPurchaseDate,
      mlmPurchaseTime,
    });
...
const addFakeUsers = async () => {
  ...
  const addUserWithDelay = async () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(addUserWithDelay, 3000);
    }, 1000);
  };
